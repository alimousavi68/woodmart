	...
	function woodmart_swatches_grid_template($attribute_name, $available_variations)
	{
		...
		foreach ($swatches_to_show as $key => $swatch) {
			...
			foreach ($available_variations as $variation) {
				...
				if (!isset($size_available[$size])) {
					$size_available[$size] = array();
				}

				if ($variation['is_in_stock']) {
					$size_available[$size][] = $color;
				}

			}

			foreach ($size_available as $size => $colors) {

				if (empty($colors)) {
					// Size $size is out of stock for all colors

					$term = get_term_by('name', $size, 'pa_size');

					if ($term) {
						// Add out of stock class to swatch term      
						$term->classes[] = 'variation-out-of-stock';
					}

				} else {
					// Size $size has at least one color available
				}
